<template>
    ...
    <div>
        <table>
            ...
            <tbody>
                <tr v-for="glavniGlumac in glavniGlumci" :key="glavniGlumac._id">
                    <td>{{ glavniGlumac.actor_id}}</td>
                    <td>{{ glavniGlumac.film_id}}</td>
                </tr>
            </tbody>
        </table>
    </div>
    <div class="sub-button">
        <button @click="otvoriDialog">DODAJ GLAVNOG GLUMCA</button>
        <ModalComponent v-if="dialogOtvoren" @zatvori="zatvoriDialog">
            <AddMainActor @zatvori="zatvoriDialog"/>
...
<script>
    ...
    export default{
        ...
        methods:{
            async dohvatiGlavneGlumce(){
                try{
                    const response=await axios.get('http://localhost:3000/glavniglumci');
                    this.glavniGlumci=response.data
                }catch(error){
